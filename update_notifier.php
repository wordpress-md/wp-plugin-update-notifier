...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
	    $xml = get_latest_plugin_version(NOTIFIER_CACHE_INTERVAL); // Get the latest remote XML file on our server
		$plugin_data = get_plugin_data(ABSPATH . 'wp-content/plugins/'. NOTIFIER_PLUGIN_FOLDER_NAME . '/' . NOTIFIER_PLUGIN_FILE_NAME .''); // Read plugin current version from the pluginfile.php

		if( (float)$xml->latest > (float)$plugin_data['Version']) { // Compare current plugin version with the remote XML version
			add_dashboard_page( NOTIFIER_PLUGIN_NAME . ' Plugin Updates', NOTIFIER_PLUGIN_NAME . ' <span class="update-plugins count-1"><span class="update-count">New Updates</span></span>', 'administrator', 'plugin-update-notifier', 'update_notifier');
		}
	}	
}
...
function update_notifier_bar_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$plugin_data['Version']) { // Compare current plugin version with the remote XML version
			$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>' . NOTIFIER_PLUGIN_NAME . ' <span id="ab-updates">New Updates</span></span>', 'href' => get_admin_url() . 'index.php?page=plugin-update-notifier' ) );
		}
	}
}
